...
/* ─── Main ChatFeed ──────────────────────────────────────── */
export default function ChatFeed({ user, profile, room, phase, secondsLeft, onMenuOpen, onBack, onCreateRoom, showToast }) {
  ...
  const handleLeave = async () => {
    const { leaveRoom } = await import("./firebase");
    await leaveRoom(room.id, user.uid);
    onBack();
    showToast?.("Left room and cleared data", "🚪");
  };
  ...
  if (room.privacy === "private" && !unlocked) return (
    <main className="chat-feed">
      <TopBar room={room} phase={phase} secondsLeft={secondsLeft} onMenu={onMenuOpen} onBack={onBack} onShare={handleShare} onLeave={() => setShowExitConfirm(true)} />
      <PwGate room={room} onUnlock={() => setUnlocked(true)} />
      {showExitConfirm && <ExitModal onConfirm={handleLeave} onCancel={() => setShowExitConfirm(false)} />}
    </main>
  );
...
/* ─── TopBar extracted ───────────────────────────────────── */
function TopBar({ room, phase, secondsLeft, onMenu, onBack, onShare, onLeave }) {
  ...
  return (
    <div className={`cf-topbar${isWarning ? " warning" : ""}`}
      ...
      <button className="cf-icon-btn cf-back-btn" onClick={onBack}>
      ...
      <div className="cf-room-info">
        ...
        <span className="cf-room-sub">{room?.memberCount || 0} participants</span>
      </div>
      <div className="cf-topbar-right">
        {secondsLeft !== null && (
          <div className="cf-expiry-wrap">
            <div className={`cf-expiry${isWarning ? " warn" : ""}`} title="Room expires in">
              {fmtTime(secondsLeft)}
            </div>
          </div>
        )}
        <button className="cf-icon-btn" onClick={() => setShowDots(!showDots)}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
            <circle cx="12" cy="5" r="1" fill="currentColor"/><circle cx="12" cy="12" r="1" fill="currentColor"/><circle cx="12" cy="19" r="1" fill="currentColor"/>
          </svg>
        </button>
        {showDots && (
          <div className="cf-dots-menu">
            <button onClick={() => { onShare(); setShowDots(false); }}>Share Link</button>
            <button onClick={() => { setShowDots(false); }}>Mute Notifications</button>
            <button className="danger" onClick={() => { onLeave(); setShowDots(false); }}>Leave Room</button>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="exit-modal-overlay">
      <div className="exit-modal">
        <h3>Leave Room?</h3>
        <p>Warning: Leaving this room will permanently delete this conversation. This action cannot be undone.</p>
        <div className="exit-modal-btns">
          <button className="exit-btn-cancel" onClick={onCancel}>Cancel</button>
          <button className="exit-btn-confirm" onClick={onConfirm}>Leave Room</button>
        </div>
      </div>
    </div>
  );
